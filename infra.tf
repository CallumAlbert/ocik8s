...
resource "oci_core_security_list" "public_subnet_s1" {
  compartment_id = var.compartment_id
  vcn_id = module.vcn.vcn_id

    display_name = "free-k8s-public-subnet-s1"
    ...
    resource "oci_core_subnet" "vcn_public_subnet" {
      ...
      security_list_ids = [oci_core_security_list.public_subnet_s1.id]
  ...
  node_source_details {
    image_id    = "ocid1.image.oc1.uk-london-1.aaaaaaaakxzengeefseyl76flumcldagsibdtujvxpgvowxs7mq7dqaxxepa"
    source_type = "image"
  }
  initial_node_labels {
    key   = "name"
